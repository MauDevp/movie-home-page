const CardMovie = ( data ) => {
    ...
    return (
        <figure key={data.data?.id} className='relative flex flex-col items-center h-full w-full'>
            ...
            />
            <span className='absolute bottom-10 right-0 md:rounded-lg rounded-2xl text-gray-200 text-lg font-medium md:font-bold md:text-xs m-2 px-2 py-0.5 bg-blue-600'>{year}</span>
            <p className='md:text-lg text-xl font-normal line-clamp-1 mb-4 md:mb-0'>{titleOrName}</p>
        </figure>
    );
}
